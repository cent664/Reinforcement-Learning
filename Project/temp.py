...
class DQNSolver:
    ...
    def act(self, state):
        ...
        weights = self.model_CNN.predict(state[0])  #TODO: Find a way to include volume and holdings in state
        features = np.concatenate(weights, state[1], state[2])
    ...
    def experience_replay(self):
        ...
        for state, action, reward, state_next, done in batch:
            ...
            if not done:
                ...
                weights = self.model_CNN.predict(state_next[0])
                features = np.concatenate(weights, state[1], state[2])
                q_update = reward + gamma * np.amax(self.model_FC.predict(features))

            weights = self.model_CNN.predict(state[0])
            features = np.concatenate(weights, state[1], state[2])
...
# TODO:
# Find a smaller static image size/some smaller representation of the image
